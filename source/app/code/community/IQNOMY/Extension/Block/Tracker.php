...
class IQNOMY_Extension_Block_Tracker extends Mage_Core_Block_Abstract
{
    ...
    protected function _toHtml()
    {
		...
		$scriptBaseLink = '//'.$this->_mediaHostAndPort[$environment].'/myliquidsuite/js/connectors/magento/';

        $html = "<script type=\"text/javascript\" src='http:".$scriptBaseLink."IQHelper.js'>"."</script>\n"
			  . "<script type=\"text/javascript\" src='http:".$scriptBaseLink."IQBanner.js'>"."</script>\n"
			  . "<script type=\"text/javascript\" src='http:".$scriptBaseLink."IQJquery.js'>"."</script>\n"
			  . "<script type=\"text/javascript\">\n"
              . "//<![CDATA[\n"
              . "if (_iqsHelper.init(". Zend_Json::encode($this->getTrackerConfig()) . ")) {\n"
	      	  . "\tvar IQUnixTime = ".time().";\n"
              . "\tvar _iqsTenant = {$tenantId};\n"
              . "\tvar _iqsImpress = { hostAndPort: '{$this->_hostAndPort[$environment]}',timeout: 5000,"
			  . ($enableCustomContainers ? "impressCallback:iqImpressCallback,backupCallback:iqImpressBackupCallback," : "")
 		      . "extra:{$enableTenantScript}};\n"
